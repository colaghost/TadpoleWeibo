...
import com.xiaotingzhong.app.WeiboConfig;
...
public class WeiboLoginWebView extends TadpoleWebView {
    ...
    public void setLoginListener(WeiboLoginListener listener) {
        mListener = listener;
    }
    ...
    private void handleNoError(Bundle values) {
        ...
        if (mToken.isSessionValid()) {
            Log.d("Weibo-authorize",
                    "Login Success! access_token=" + mToken.getToken() + " expires="
                            + mToken.getExpiresTime() + " refresh_token="
                            + mToken.getRefreshToken());
            mListener.onComplete(mToken);
        } else {
            Log.d("Weibo-authorize", "Failed to receive access token");
            mListener.onWeiboException(new WeiboException("Failed to receive access token."));
        }
